# Roadmap: Festlegungen und datierte Nachweise

Verbindliche Reihenfolge: [Roadmap](roadmap.md). Dieses Dokument wird nur für das
aktuell betroffene Paket gelesen. Statusangaben sind datierte Nachweise, keine
aktuelle Produktionsprüfung; historische Schrittnummern bestimmen keine Priorität.
...
| Aktueller Release-Nachweis | Lokal, isoliert und technisch produktiv bestätigt | Ergebnisse und Grenzen im [aktuellen Preflight-Bericht](</Users/vladislavhirschfeld/Documents/Trainer App.worktrees/step-5-training/trainer-webapp/docs/release-1-preflight-2026-09-08.md>); fachliche Praxisabnahme nachführen. |
...
## Fachlicher Umfang der Pakete

| Schritt | Funktionspaket | Kernumfang und Nutzen |
| ---: | --- | --- |
...
| 5 | Persistente Trainingsgrundlage und Trainingsmodus | Eigene Pläne und Versionen dauerhaft speichern. Danach tatsächliche Anwesenheit getrennt von Zusagen, Übungen, Timer, Notizen sowie Versuche und Landungen je Trick in einer eindeutig referenzierten Session erfassen. Erfolgsquoten entstehen aus realen Sessions statt nur aus Planstatus. |
| 6 | Session-Abschluss und Fortschrittsverlauf | Aus den Trainingsdaten einen prüfbaren Session-Recap mit bearbeiteten Skills, Versuchen, Landungen, Trainerhinweisen, nächsten Zielen und bestätigten Fortschritten erzeugen. Athleten und verknüpfte Eltern sehen eine kompakte Zusammenfassung. |
| 7 | Minimalistischer 3D-Park- und Run-Planer | Skateparks als einfache Low-Poly-Modelle aus einer Obstacle-Bibliothek aufbauen; optional GLB/glTF oder Heightmaps importieren. Tricks an stabile Obstacles anpinnen, Richtung und Reihenfolge festlegen sowie Runs Athleten, Trainings oder Contests zuweisen. |
| 8 | Strategische Run-Auswertung und Scores | Erwarteten beziehungsweise angestrebten Score und den tatsächlich erhaltenen Score pro Run speichern. Run-Varianten, Trickfolge, Obstacles und historische Ergebnisse vergleichbar machen, damit Athleten Risiko und Punktepotenzial strategisch abwägen können. Vertiefte Wertungskriterien bleiben standardmäßig eingeklappt. |
| 9 | Strukturiertes Curriculum | Skill-Pakete, Voraussetzungen, Progressionslevel, Session-Vorlagen und Versionierung auf Vereins-, Landes- und Bundesebene aufbauen. Lokale Anpassungen bleiben möglich, ohne den freigegebenen Standard zu überschreiben. |
| 10 | Contest Hub und Live-Wertung | Startlisten, Run-Zuordnung, Judge-Modus, mehrere Wertungsrichter, Korrekturen, Gleichstand, Ausfälle, Live-Ergebnisse und Übernahme in Athletenprofile. Street unterstützt Run, 2/5/3 sowie Run-Qualifikation mit 2/5/3-Finale; Park unterstützt Run. Run-Runden besitzen standardmäßig zwei Runs, von denen der bessere zählt; mehr Runs liegen in erweiterten Einstellungen. |
| 11 | Familienübersicht und Teilnahmeorganisation | Dauerhafte Kinderprofile zunächst ohne eigenen Login. Eltern können eigene Kinder anlegen; ausdrücklich berechtigte Vereinsrollen können minimale Kinderprofile für ihren Verein erfassen. Geprüfte Beziehungen steuern den Zugriff. Mehrere Kinder und offene Aufgaben bilden den ersten Teil; Abwesenheiten und Trainingswarteliste folgen als getrennte Abläufe. |
| 12 | Offline-Unterstützung für den Trainingsort | Trainingsplan, Teilnehmerliste und laufende Session ohne stabile Verbindung nutzbar machen; Anwesenheit, Versuche, Landungen und Notizen anschließend konfliktarm synchronisieren. Stabile IDs, Revisionen und Konfliktregeln werden bereits mit dem Sessionmodell festgelegt. |
| 13 | Eigener Video-Workflow | Nach dem Offline-Trainingsablauf Videos direkt aufnehmen oder hochladen, Training, Athlet, Trick und später einem Park-Obstacle zuordnen; Trainerprüfung, Textfeedback und sichere Zugriffsregeln. Zeitmarken, Zeichnungen, Sprachfeedback und optionale KI-Vorschläge folgen als vertiefte Werkzeuge. |
...
## Arbeitsschritt 5: Persistente Trainingsgrundlage und Trainingsmodus

Der Nutzer hat am 23.09.2026 den getrennten Athleten-/Trainerentwurf mit
Fahrerslider freigegeben und die Umsetzung beauftragt. Bestätigt sind
persönliches Training ohne Verein, betreutes Einzel-/Gruppentraining durch
aktiv zugeordnete Trainer, unveränderliche abgeschlossene Sessions sowie
...
[Prüfkontext, Grenzen und Rollback](</Users/vladislavhirschfeld/Documents/Trainer App.worktrees/step-5-training/trainer-webapp/docs/training-step-5-release.md>).
Schritt 5 wurde vom Nutzer am 24.09.2026 fachlich abgenommen. Diese Bestätigung
ergänzt den damaligen technischen Nachweis; sie behauptet keine erneute Prüfung.
...
- **Rollen:** Athleten nur eigene Ergebnisse, Trainer mit aktiver bestehender
  Zuordnung, Eltern ausschließlich über bestehende aktive gerichtete
  `guardian`-Beziehungen. Keine neue Anlage oder Verwaltung von Beziehungen.
- **Umfang:** Versuche ohne rückgängig gemachte Einträge, Landungen, gewichtete
  Erfolgsquoten, Session-Zeitraum einschließlich Pausen und gemessene Übungstimer.
  Gruppenzeit ist keine persönliche Trainingsdauer. Ohne Versuche keine Quote
  und kein behaupteter persönlicher Bearbeitungsnachweis. Verlauf nur aus
  abgeschlossenen gespeicherten Sessions; stabile Quell-Skill-IDs werden je Athlet
  zusammengefasst. Unterschiedliche IDs werden nicht über Namensähnlichkeit vereint.
- **Bestätigte Regeln:** Anfrage und ausdrückliche Skill-Bestätigung bleiben von
  Landungen getrennt. Ohne zugeordneten Trainer darf der Athlet selbst bestätigen;
  sonst bestätigen aktuell zugeordnete Trainer. Alle aktuell zugeordneten Trainer
  dürfen persönliche Hinweise und nächste Ziele ergänzen. Korrekturen werden als
  neue Einträge mit Verweis, Autor, Rolle und Zeitpunkt gespeichert; alte Einträge
  bleiben sichtbar. Ergänzungen verändern weder Rohdaten noch alte Planfortschritte.
- **Notizen:** allgemeine betreute Session-/Übungs-/Planhinweise bleiben beim
  bisherigen berechtigten Session-Verantwortlichen. Athleten und Eltern sehen
  persönliche Ergebnisse und personenbezogene Ergänzungen. Notizen aus eigenem
  Training bleiben im persönlichen Recap sichtbar. Keine Gruppenlisten oder
  vollständigen Plan-Snapshots im neuen Abruf.
- **Qualitätsrahmen ISO/IEC 25002:** funktionale Richtigkeit (Zähler, Nullquote,
  stabile Zuordnung), Vertraulichkeit (personenbezogene DB-Projektion), Integrität
  und Nachvollziehbarkeit (unveränderliche Trainingsdaten, ergänzende Historie),
  Verständlichkeit (Zeitarten, Bestätigung und Landung getrennt).
- **Abnahmekriterien:** Werte stimmen mit erfassten Sessions überein; fremde
  Gruppenergebnisse bleiben unsichtbar; entzogene Beziehungen entziehen Zugriff;
  nur zulässige Personen können ergänzen/bestätigen; Korrekturhistorie bleibt
  erhalten; erneutes Laden zeigt gespeicherte Daten.
- **Ausschlüsse:** 3D-Planer, Contest-Wertung, Curriculum, Familienverwaltung,
  Offline-Synchronisierung, neue Videos, Chat und Push.
- **Prüfverfahren:** auf ausdrückliche Nutzeranweisung ausschließlich erforderlicher
  Deployment-Build. Keine automatisierten Tests, kein Lint, keine separate
  Typprüfung, keine Browser-, Datenbank-Funktions- oder Rollenprüfung und keine
  neuen Tests. Eine vom Framework integrierte Build-Typprüfung bleibt Bestandteil
  des Builds. Funktions- und Praxisabnahme übernimmt der Nutzer.

### Praxisprüfliste Schritt 6

1. Eigene Session abschließen, Recap öffnen und Versuche, Rückgängig, Landungen,
   vorhandene Notizen und Zeiten mit der Erfassung vergleichen; Seite neu laden.
2. Übung ohne Versuche: keine Prozentquote; Landungen erzeugen keine Bestätigung.
3. Mehrere Sessions desselben Skills: Verlauf und Gesamtquote aus Summen prüfen.
4. Gruppentraining: zwei Athletenkonten sehen jeweils nur eigene Werte, keine
   anderen Teilnehmer oder allgemeinen Gruppennotizen; Gruppenzeit ist beschriftet.
5. Trainer mit aktiver Zuordnung ergänzt Hinweis/Ziel; anderer aktiver Trainer
   korrigiert durch neuen Eintrag. Autor, Zeitpunkt und Original bleiben sichtbar.
6. Athlet fragt Bestätigung an. Mit Trainer bestätigt dieser; ohne Trainer kann
   der Athlet selbst bestätigen. Eltern dürfen weder ergänzen noch bestätigen.
7. Bereits verknüpftes Elternkonto sieht nur Zusammenfassungen seiner Kinder.
   Nicht verknüpfte Konten und entzogene Beziehungen erhalten keine Ergebnisse.
...
[Technischen Bericht für Schritt 2](</Users/vladislavhirschfeld/Documents/Trainer App.worktrees/step-5-training/trainer-webapp/docs/auth-onboarding-step-2-technical-report-2026-09-09.md>).
Der vollständige fachliche Ablauf und die abgeleiteten Zustände stehen in
[Onboarding-Ablauf für Schritt 2](</Users/vladislavhirschfeld/Documents/Trainer App.worktrees/step-5-training/trainer-webapp/docs/onboarding-flow-step-2.md>).
...
[Technischen Bericht für Schritt 3](</Users/vladislavhirschfeld/Documents/Trainer App.worktrees/step-5-training/trainer-webapp/docs/password-management-step-3-technical-report-2026-09-22.md>).
...
Schritt 2 ist vom späteren umfangreichen Trainingspaket in Schritt 5 abzugrenzen.
